...
public class MySQLConnection implements DBConnection{
	...
	/**
	 * Return all ads where their bid amount is > 0 as candidates
	 * @return
	 */
	...
	public void updateBudget(int advertiser_id, double budget) {
		...
		System.out.println("update budget succeeded");
	}
	...
	public void updateBid(int ad_id, double bid) {
		...
		System.out.println("update bid succeeded");
	}
		...
		try {	
	        ...
	        try (ResultSet generatedKeys = stmt.getGeneratedKeys()) {
	            ...
	            if (generatedKeys.next()) {
	            	// return the first parameter (1), which is a long type
	...
	public long createAd(double bid, String image_url, int advertiser_id, double ad_score) {
		...
		try {
			...
			System.out.println("inserting stmt: "+stmt.toString());
